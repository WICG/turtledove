...
All fields that accept arbitrary metadata objects (`userBiddingSignals` and `metadata` field of ads) must be JSON-serializable values (which may include primitive values, arrays, objects of arbitrary structure).
...
All fields that accept arbitrary metadata objects (`auctionSignals`, `sellerSignals`, and values of `perBuyerSignals` dictionary) must be JSON-serializable values (which may include primitive values, arrays, objects of arbitrary structure).
