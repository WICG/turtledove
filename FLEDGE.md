...
Optionally, `sellerTimeout` can be specified to restrict the runtime (in milliseconds) of the seller's script (`scoreAd()` and `reportResult()`), and `perBuyerTimeouts` can be specified to restrict the runtime (in milliseconds) of particular buyer's bidding scripts (`generateBid()` and `reportWin()`). If no value is specified for the seller or a particular buyer, a default timeout of 50 ms will be selected. Any timeout higher than 500 ms will be clamped to 500 ms. A key of `'*'` in `perBuyerTimeouts` is used to change the default of unspecified buyers.
