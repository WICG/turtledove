...
*   browserSignals: Similar to the argument to `reportResult()` above, though without the seller's desirability score, but with additional `adCost`, `seller`, `madeHighestScoringOtherBid` and potentially `interestGroupName` fields.
    *   The `adCost` field contains the value that was returned by `generateBid` (or the bid value if no value was specified), stochastically rounded to fit into a floating point number with an 8 bit mantissa and 8 bit exponent.
