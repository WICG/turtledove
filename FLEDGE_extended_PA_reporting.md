...
(i.e. the interest group’s owner was listed in the `interestGroupBuyers` -- note the count *isn't* limited by the auction config's `perBuyerGroupLimits`). This requires the `interestGroupCounts` `sellerCapabilities` permission.
